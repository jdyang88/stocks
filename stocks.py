...
# Streamlit app title
st.title('Stock Price Prediction Dashboard with Sentiment Analysis')
...
# Fetching news articles using NewsAPI for sentiment analysis
news_api_key = os.environ.get('c4b101fd9551431cadbae50314fba17c')  # Add your API key as an environment variable
newsapi = NewsApiClient(api_key=news_api_key)

# Fetch news articles related to the selected company
articles = newsapi.get_everything(q=selected_company.split(' (')[0], language='en', sort_by='relevancy', page_size=100)
...
# Incorporate sentiment score into the stock data
sentiment_feature = np.full((df.shape[0], 1), avg_sentiment)
df['Sentiment'] = sentiment_feature
...
# LSTM Model Execution Button
if st.button('Run LSTM Model to predict future price with sentiment analysis'):
    st.write("*Modeling in progress. Please wait...*")
    # Initialize a progress bar
    progress_bar = st.progress(0)
    ...
    # Display future price predictions
    for day, future_price in enumerate(future_prices, start=1):
        st.write(f"{day} days later {selected_company.split(' (')[0]} Stock Price Prediction: {future_price:.2f}")

    # Display Prediction Graphs
    plt.figure(figsize=(14, 7))
    plt.plot(scaler.inverse_transform(dataset)[:,0], label='Actual Stock Price')
    plt.plot(np.arange(len(dataset), len(dataset)+60), future_prices, label='Predicted Future Prices')
    plt.xlabel('Time')
    plt.ylabel('Stock Price')
    plt.title(f'Future Stock Price Predictions for {selected_company.split(" (")[0]} with Sentiment Analysis')
    plt.legend()
    plt.grid()
    st.pyplot(plt)
